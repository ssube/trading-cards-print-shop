...
export function CardLibrary({ cards, catalog, cardProgress, onOpenCard, onVisitPress, selectedType, onSelectType }: {
  cards: CardCopy[]
  catalog: Part[]
  ...
  return <section className="page library-page"><div className="page-intro row-intro"><div><p className="eyebrow">YOUR PRIVATE ARCHIVE</p><h1>{complete ? <>Your collection, <em>completed.</em></> : <>The card <em>library.</em></>}</h1><p>Every copy has a story. Open the box and take one out.</p></div><div className="collection-count"><strong>{cards.length}</strong><span>CARDS IN YOUR BOX</span></div></div>
    <div className="box-tabs" role="group" aria-label="Filter cards by type">
      <button type="button" className={selectedType === 'all' ? 'active' : ''} aria-pressed={selectedType === 'all'} onClick={() => onSelectType('all')}>ALL CARDS <b>{cards.length}</b></button>
      {types.map(type => <button type="button" key={type} className={selectedType === type ? 'active' : ''} aria-pressed={selectedType === type} onClick={() => onSelectType(type)}>{(typeNames.get(type) || type).toUpperCase()} <b>{cards.filter(card => card.type_id === type).length}</b></button>)}
    </div>
