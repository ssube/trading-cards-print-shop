...
export function CardLibrary({ cards, catalog, onOpenCard, onVisitPress }: {
  cards: CardCopy[]
  catalog: Part[]
  ...
  return <section className="page library-page"><div className="page-intro row-intro"><div><p className="eyebrow">YOUR PRIVATE ARCHIVE</p><h1>The card <em>library.</em></h1><p>Every copy has a story. Open the box and take one out.</p></div><div className="collection-count"><strong>{cards.length}</strong><span>CARDS IN YOUR BOX</span></div></div>
    ...
    {visibleCards.length ? <div className="card-box"><div className="box-lid"><div>✦ &nbsp; THE PERSONAL ARCHIVE &nbsp; ✦</div></div><div className="box-inner">{visibleCards.map(card => <div className="boxed-card" key={card.id}><Card card={card} onClick={() => onOpenCard(card)} /><div className="box-card-label"><strong>{card.name}</strong><span>{card.slab_grade !== null ? `GRADED ${card.slab_grade}` : card.estimated_grade} · {card.finish_id}</span></div></div>)}</div><div className="box-foot">HANDLE WITH CARE · PRINTED WITH WONDER</div></div>
