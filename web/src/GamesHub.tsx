...
export function GamesHub({ onChanged }: { onChanged: () => Promise<void> }) {
  ...
  return <section className="page games-page">{selected ? <><button type="button" className="secondary" onClick={() => setSelected(null)}>← All games</button>{selected === 'papermill' && <PapermillGame onChanged={onChanged} />}{selected === 'fishing' && <FishingGame onChanged={onChanged} />}{selected === 'shooter' && <ShooterGame onReward={onChanged} />}</> : <><div className="page-intro"><p className="eyebrow">BEYOND THE PRINTING PRESS</p><h1>Games in the <em>works.</em></h1><p>More ways to pass the time between prints are on the workshop calendar.</p></div><div className="games-grid">{games.map(game => <article className="game-stub" key={game.id}><div className="game-stub-top"><span className="game-stub-icon" aria-hidden="true">{game.icon}</span><span className="game-stub-status">{game.ready ? 'NOW PLAYING' : 'COMING SOON'}</span></div><h2>{game.title}</h2><p>{game.description}</p>{game.ready && <button className="secondary" onClick={() => setSelected(game.id)}>Play ↗</button>}</article>)}</div></>}</section>
}
