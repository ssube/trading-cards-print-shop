...
import type { CardCopy, CollectionProgress as Progress, Listing, NpcOffer, StarterDeck, State, User } from './types'

type Tab = 'workshop' | 'finishes' | 'library' | 'progress' | 'commissions' | 'trading' | 'admin'
...
function App() {
  ...
  async function authenticate() {
    ...
    setAuthBusy(true)
    try {
      const u = await api<User>(`/auth/${authMode}`, 'POST', authMode === 'register' ? { username, password, starter_deck_id: starterDeckId } : { username, password })
  ...
  if (!user) return <StarterWelcome mode={authMode} setMode={setAuthMode} decks={starterDecks} selectedDeck={starterDeckId} setSelectedDeck={setStarterDeckId} username={username} setUsername={setUsername} password={password} setPassword={setPassword} message={message} busy={authBusy} onSubmit={authenticate} />

  return <div className="app-shell"><aside className="sidebar"><div className="brand"><div className="brand-seal">C<span>:</span>P</div><div><strong>Cards:</strong><small>the Printing</small></div></div><div className="sidebar-caption">COLLECTOR'S WORKSHOP</div><nav>{nav.map(item => <button key={item.key} className={tab === item.key ? 'active' : ''} onClick={() => { setTab(item.key); setMessage(''); if (item.key === 'admin') api<typeof adminInfo>('/admin/overview').then(setAdminInfo).catch(e => setMessage(e.message)) }}><span>{item.icon}</span>{item.label}</button>)}</nav><div className="sidebar-bottom"><div className="little-star">✦</div><p>“A good card is never finished. Only printed.”</p><div className="profile"><span className="avatar">{user.username[0].toUpperCase()}</span><div><strong>{user.username}</strong><small>Collector #{String(user.id).padStart(3, '0')} · {starterDecks.find(deck => deck.id === user.starter_deck_id)?.name || 'Founding edition'}</small></div><button title="Sign out" onClick={() => run(async () => { await api('/auth/logout', 'POST'); setUser(null) }, '')}>↪</button></div></div></aside>
    ...
    {tab === 'progress' && <ProgressPage state={state} onOpenCard={setSelected} onNavigate={setTab} />}
