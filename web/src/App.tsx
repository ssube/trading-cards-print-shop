...
function App() {
  ...
  function openCard(card: CardCopy) { setSelected(card); navigate(tab, 'card', card.id) }
  function closeCard() { setSelected(null); navigate(tab) }
  useEffect(() => {
  ...
  useEffect(() => {
    if (!selected) return
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') closeCard() }
