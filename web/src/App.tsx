...
function App() {
  ...
  async function printCard() {
    ...
    try {
      const job = await api<{ id: string; discovery_name?: string }>('/prints', 'POST', { type_id: recipe.type_id, rule_ids: rules, theme_id: recipe.theme_id, finish_id: finish, border_id: recipe.border_id, back_id: recipe.back_id, foil_back: recipe.foil_back, hint: titleHint.trim() }, { 'Idempotency-Key': uniqueId() })
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><div className="brand-seal">TC<span>:</span>PS</div><div><strong>Trading Cards:</strong><small>Print Shop</small></div></div>{offline && <div className="offline-sidebar-label">◌ OFFLINE DEMO</div>}<div className="sidebar-caption">COLLECTOR'S WORKSHOP</div><nav>{nav.map(item => item.disabled ? <button key={item.key} type="button" disabled title="Online only"><span>{item.icon}</span>{item.label}<small className="nav-unavailable">ONLINE ONLY</small></button> : <a key={item.key} href={routeUrl(item.key)} className={tab === item.key ? 'active' : ''} aria-current={tab === item.key ? 'page' : undefined} onClick={() => { if (item.key === 'print') setPrintPreset(null); setMessage('') }}><span>{item.icon}</span>{item.label}</a>)}</nav><div className="sidebar-bottom"><div className="little-star">✦</div><p>“A good card is never finished. Only printed.”</p><div className="profile"><span className="avatar">{user.username[0].toUpperCase()}</span><div><strong>{user.username}</strong><small>{offline ? 'Saved in this browser' : `Collector #${String(user.id).padStart(3, '0')} · ${starterDecks.find(deck => deck.id === user.starter_deck_id)?.name || 'Founding edition'}`}</small></div>{!offline && <button title="Sign out" onClick={() => run(async () => { await api('/auth/logout', 'POST'); setUser(null) }, '')}>↪</button>}</div></div></aside>
    <main className="main-content"><header className="topbar"><span className="breadcrumb">THE ARCHIVE <span>/</span> {nav.find(n => n.key === tab)?.label.toUpperCase()}</span><div className="topbar-right">{!state.allowance_claimed && <button type="button" className="topbar-allowance" onClick={claimDailySupplies}>✦ Collect daily supplies</button>}<a className="resource-pill" href={routeUrl('progress', 'resource', 'paper')}>▤ <strong>{state.resources.paper || 0}</strong> paper</a><a className="resource-pill" href={routeUrl('progress', 'resource', 'ink')}>◉ <strong>{state.resources.ink || 0}</strong> ink</a><a className="resource-pill foil-pill" href={routeUrl('progress', 'resource', 'foil')}>✧ <strong>{state.resources.foil || 0}</strong> foil</a><a className="resource-pill" href={routeUrl('progress', 'resource', 'sleeve')}>◇ <strong>{state.resources.sleeve || 0}</strong> sleeves</a></div></header>
