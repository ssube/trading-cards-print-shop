...
export function TabletopGame({ state, onChanged, offline = false }: { state: State; onChanged: () => Promise<void>; offline?: boolean }) {
  ...
  const practiceCopy = state.library.find(card => card.id === practice?.copy_id)
  ...
  return <section className="page tabletop-page">
    ...
    <section className="tabletop-practice"><div><p className="eyebrow">SOLO PRACTICE</p><h2>{practice?.step === 3 ? 'You know your way around the table.' : lessons[practice?.step || 0].title}</h2><p>{practice?.step === 3 ? 'All three lesson cards are in your collection.' : lessons[practice?.step || 0].detail}</p></div><div className="tabletop-practice-actions">{practiceCopy && <div className="tabletop-practice-card" aria-label={(practice?.step ?? 0) >= 2 ? 'Practice card face down' : practiceCopy.name}>{(practice?.step ?? 0) >= 2 ? <strong>TC:PS</strong> : <img src={practiceCopy.art_path} alt={practiceCopy.name} />}{practice?.step === 3 && <b>1</b>}</div>}<span>{practice ? `${practice.step} / 3 lessons` : 'Loading lessons…'}</span>{practice?.next_action && <button className="primary" disabled={busy || practice.step === 0 && !selected.length} onClick={() => void practiceAction()}>{practice.next_action === 'place' ? 'Place chosen card' : practice.next_action === 'flip' ? 'Flip practice card' : 'Add a counter'} ↗</button>}</div></section>
