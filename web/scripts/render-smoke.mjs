...
if (!gamesMarkup.includes('Feline Papermill') || !gamesMarkup.includes('Every gacha game needs a fishing minigame') || !gamesMarkup.includes('Wait, you can play with these cards?') || !gamesMarkup.includes('Pressroom Inferno') || (gamesMarkup.match(/NOW PLAYING/g) || []).length !== 4 || !printMarkup.includes('Loaded from') || !printMarkup.includes('1 card placements') || !printMarkup.includes('Show simulated foil finish') || !printMarkup.includes('Show print defects and paper wear') || !appMarkup.includes('Warming the press') || !cardMarkup.includes('Apprentice Press Cat') || !cardMarkup.includes('art-window') || !cardMarkup.includes('TC') || !cardMarkup.includes('PRINT SHOP') ||
